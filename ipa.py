...
Original file is located at
    https://colab.research.google.com/drive/1DNqsGx_IwRgbMWpatVficF-3__3xymZv

*italicized text*#***ML Project***

### **Problem Statement:**
**We have a list of trades of different simulations (eash is numbered and based on different settings). Each simulation represents a different permutation of settings used to run them. These are results of a trading algorithm which decides when and how to buy or sell different assets.
We want to know what permutation will work for Wednesday 15:00 (for example) and we assume that each day of the week and hour of the day has a different profile as certain activities are based on news which are the same hours of the days. etc.**

**Selecting the best permutation (best by aggregated profit & loss) for a future period is needed by selecting day of the week and time of the day..
The different ML method / model is upon your selection.
All work will be done on google colab and need to use**

### **Approach to Problem:**
**My Aim was to build a Super Learner model which consists of  all the possible Regression models (for now due to time constraint:3) , with an algorithm  which is primarly designed to extract the best performing model  from the list of sub models in the super learner model and then make the OOS predictions with aggregate profit/loss calculation along with the percentage of good trading days are predicted by the alogrithm for the same.**

### **Soln:**
**I have broken the entire algorithm into three stages where each stage performs a specified function.**

**Stage:1- Selecting the best permutation out of the sixteen different types of permutation.**

**Stage:2- Profit/Loss analysis using the best outperforming model , also the validity of the best model (i.e number of days would be indicated).**

**Stage-3: Integeration of Stage-1 and Stage-2 with also addition of the data pre processing steps required to perform Stage-1 and Stage-2 analysis.**
...
"""**Peeking the contents of the folder CV task**"""

for dirname, _, filenames in os.walk('/content'):
    for filename in filenames:
        print(os.path.join(dirname, filename))

"""**Loading all the different files into colab**

**Data Preprocessing**
"""

df1 = pd.read_csv('/content/GC AT IPS(2)+TM(A) 2016-05_2021 sim131 Trades.txt',delimiter="\t")
df1['SIM']="sim131"
df1=df1.dropna(how='any')
df1

df2 = pd.read_csv('/content/GC AT IPS(2)+TM(A) 2016-05_2021 sim133 Trades.txt',delimiter="\t")
df2['SIM']="sim133"
df2=df2.dropna(how='any')
df2

df3 = pd.read_csv('/content/GC AT IPS(2)+TM(A) 2016-05_2021 sim113 Trades.txt',delimiter="\t")
df3['SIM']="sim113"
df3=df3.dropna(how='any')
df3

df4 = pd.read_csv('/content/GC AT IPS(2)+TM(A) 2016-05_2021 sim112 Trades.txt',delimiter="\t")
df4['SIM']="sim112"
df4=df4.dropna(how='any')
df4

df5 = pd.read_csv('/content/GC AT IPS(2)+TM(A) 2016-05_2021 sim122 Trades.txt',delimiter="\t")
df5['SIM']="sim122"
df5=df5.dropna(how='any')
df5

df6 = pd.read_csv('/content/GC AT IPS(2)+TM(A) 2016-05_2021 sim101 Trades.txt',delimiter="\t")
df6['SIM']="sim101"
df6=df6.dropna(how='any')
df6

df7 = pd.read_csv('/content/GC AT IPS(2)+TM(A) 2016-05_2021 sim123 Trades.txt',delimiter="\t")
df7['SIM']="sim123"
df7=df7.dropna(how='any')
df7

df8 = pd.read_csv('/content/GC AT IPS(2)+TM(A) 2016-05_2021 sim110 Trades.txt',delimiter="\t")
df8['SIM']="sim110"
df8=df8.dropna(how='any')
df8

df9 = pd.read_csv('/content/GC AT IPS(2)+TM(A) 2016-05_2021 sim132 Trades.txt',delimiter="\t")
df9['SIM']="sim132"
df9=df9.dropna(how='any')
df9

df10 = pd.read_csv('/content/GC AT IPS(2)+TM(A) 2016-05_2021 sim130 Trades.txt',delimiter="\t")
df10['SIM']="sim130"
df10=df10.dropna(how='any')
df10

df11 = pd.read_csv('/content/GC AT IPS(2)+TM(A) 2016-05_2021 sim103 Trades.txt',delimiter="\t")
df11['SIM']="sim103"
df11=df11.dropna(how='any')
df11

df12 = pd.read_csv('/content/GC AT IPS(2)+TM(A) 2016-05_2021 sim100 Trades.txt',delimiter="\t")
df12['SIM']="sim100"
df12=df12.dropna(how='any')
df12

df13 = pd.read_csv('/content/GC AT IPS(2)+TM(A) 2016-05_2021 sim102 Trades.txt',delimiter="\t")
df13['SIM']="sim102"
df13=df13.dropna(how='any')
df13

df14 = pd.read_csv('/content/GC AT IPS(2)+TM(A) 2016-05_2021 sim120 Trades.txt',delimiter="\t")
df14['SIM']="sim120"
df14=df14.dropna(how='any')
df14

df15 = pd.read_csv('/content/GC AT IPS(2)+TM(A) 2016-05_2021 sim111 Trades.txt',delimiter="\t")
df15['SIM']="sim111"
df15=df15.dropna(how='any')
df15

df16 = pd.read_csv('/content/GC AT IPS(2)+TM(A) 2016-05_2021 sim121 Trades.txt',delimiter="\t")
df16['SIM']="sim121"
df16=df16.dropna(how='any')
df16

"""**Combining all the sixteen different preprocessed dataframes into a single dataframe**"""
...
"""**Processing the main dataframe contents**"""

df['Entry DateTime'] = pd.to_datetime(df['Entry DateTime'])
df['Exit DateTime'] = pd.to_datetime(df['Exit DateTime'])

df['Day']=pd.to_datetime(df['Entry DateTime']).dt.day
df['Month']=pd.to_datetime(df['Entry DateTime']).dt.month
df['year']=pd.to_datetime(df['Entry DateTime']).dt.year
df['Time(Entry)']=pd.to_datetime(df['Entry DateTime']).dt.time
#df['Time(Exit)']=pd.to_datetime(df['Exit DateTime']).dt.time
df['DOW']=pd.to_datetime(df['Entry DateTime']).dt.day_name()#Type of day of the week
df["Entry DateTime"]=df["Entry DateTime"].dt.strftime("%y-%m-%d")

df['Time(Entry)']=df['Time(Entry)'].astype(str)
df['Time(Entry)']=(df['Time(Entry)'].str.split(':').apply(lambda x: int(x[0]) * 60 + int(x[1]))).astype(int)
df['Duration']=df['Duration'].astype(str)
df['Duration']=(df['Duration'].str.split(':').apply(lambda x: int(x[0]) * 60 + int(x[1]))).astype(int)

df=df.drop(columns=['Symbol','Trade Type','Exit DateTime','Max Open Quantity','Max Closed Quantity','Cumulative Profit/Loss (C)','Exit Efficiency','Entry Efficiency','FlatToFlat Profit/Loss (C)','FlatToFlat Max Open Profit (C)','FlatToFlat Max Open Loss (C)','Max Open Profit (C)','Max Open Loss (C)','Note','Commission (C)','Open Position Quantity','Close Position Quantity',])

df=df.rename(columns ={'Entry DateTime':'Date',}, inplace = False)
df['Total Efficiency'] = list(map(lambda x: x[:-1], df['Total Efficiency'].values))
df['Total Efficiency'] = [float(x) for x in df['Total Efficiency'].values]
df['Time(Entry)']=df['Time(Entry)'].astype(int)
df

df['DOW'].unique()

"""##### **Condition:Based on the above code cell we can concur that all the dates when mapped to the days of the week given in the main dataframe are devoid of Saturday so hence its prudent to showcase that due to the absence of Saturday the algorithm would not respond to the same. Apart from this the timings for the day "Sunday" would only start from 1000 minutes and not less than that due to the absence of the data in the datasets given for the same.**

**Hence it is advisable not to provide Sunday and Saturday as inputs to the algorithm as it would fail to perform due to lack of adequate data for the same.**
...
df1,df2,df3,df4,df5,df6,df7,df8,df9,df10,df11,df12,df13,df14,df15,df16=process()

"""##**Important Note:** 


### **Kindly do adhere to the condition (Condition) mentioned above when prompted to provide inputs to the cells throughout the notebook**

### **Thank you**

### **Stage-1:**
**Selecting the best permutation  for the training of model done by selecting day of the week and time of the day based on problem statement**
...
"""### **The below two code cells are dedicated to rseult with the best permutation having high efficiency for the day**
...
# ouptut: returns the best permutaion dataframe based on simulator type 
  ...
  bp['Time(Entry)']=bp['Time(Entry)'].astype(str)
  bp['Time(Entry)']=(bp['Time(Entry)'].str.split(':').apply(lambda x: int(x[0]) * 60 + int(x[1]))).astype(int)
  bp=bp.drop(columns=['Symbol','Trade Type','Exit DateTime','Duration','Profit/Loss (C)','Total Efficiency','Time(Entry)','Max Open Quantity','Max Closed Quantity','Cumulative Profit/Loss (C)','Exit Efficiency','Entry Efficiency','FlatToFlat Profit/Loss (C)','FlatToFlat Max Open Profit (C)','FlatToFlat Max Open Loss (C)','Max Open Profit (C)','Max Open Loss (C)','Note','Commission (C)','Open Position Quantity','Close Position Quantity',])
  bp=bp.rename(columns ={'Entry DateTime':'Date','Entry Price':'Open','Exit Price':'Close','High Price While Open':'High','Low Price While Open':'Low','Trade Quantity':'Volume'}, inplace = False)
  #bp['Total Efficiency'] = list(map(lambda x: x[:-1], bp['Total Efficiency'].values))
  #bp['Total Efficiency'] = [float(x) for x in bp['Total Efficiency'].values]
  #bp['Time(Entry)']=bp['Time(Entry)'].astype(int)
...
"""**Concept used for gathering time from the above code cell is illustrated below**"""
...
**Processing the best permutation dataset for model prediction**

**Note: As Stage-1 and Stage-2 are not yet integerated hence kindly change the name of the best permutation csv file recieved at your end after performing Stage-1 analysis**
"""

bp=pd.read_csv("Best Permutation file sim112.csv",index_col=False)
...
bp = bp.reindex(['Open','High','Low','Close','Volume','Profit/Loss (C)'], axis=1) #Gathering extra technical features which can be used for predicting the OOS profit/loss

bp

"""**Ensuring Correct Format of Date**"""
...
**In Stage-2 ,I have taken the liberty to train a super learner model i.e a model which is a combination of all the possible regression models which are made to train and predict OOS in a Time Series Dataset(Best Permutation Dataset)**
...
**1)Since this algorithm needs to function on a hourly basis in a day hence certain models like (ANN,Adaboost Regressor,Bagging Regressor,Support Vector Regression,Extra Trees Regressor and Random Forest Regression) which could outperform the existing models couldnot be used due to time taken for training of the model is high(i.e suppose if the model algorithm is initialised now, then the training of these models for the allocated data would take maybe 30-45 mins and there is a possibilty that the algorithm would have considerable amount of backlogues due to hourly refresh period) hence for the time being they have been removed or made inactive but if the design criteria changes then definetly they could be added for the same.**

**2)These algorithms are designed to function on an hourly basis as mentioned**

**Note: During the execution of the super learner without cross validation model kindly execute the above 9 cells so as to provide a clean bug free data to the super learner model**
...
"""**Data preprocessing for model with cross validation**"""

bp=pd.read_csv("Best Permutation file sim102.csv",index_col=False)
bp=bp.set_index('Date')
...
bp = add_all_ta_features(bp, "Open", "High", "Low", "Close", "Volume", fillna=True) #Gathering extra technical features which can be used for predicting the OOS profit/loss along with present profit and loss values
...
"""### **Trading Analysis without using Profit/Loss(C) as input**

**Without cross validation**
"""

bp=pd.read_csv("Best Permutation file sim131.csv",index_col=False)
bp=bp.set_index('Date')

bp

bp = bp.reindex(['Open','High','Low','Close','Volume'], axis=1)
...
"""**Repeating the steps of Stage-2 Preprocessing for feeding the proceesed dataframe to the SuperLearner using Cross Validation**
...
"""**Note: During the execution of the super learner with cross validation model kindly execute the above 7 cells so as to provide a clean bug free data to the super learner model**

**With Cross Validaton**
"""
    ...
    disp_results(sfd,pa,tpd)
      ...
      for j in shft:
        ...
        if j<=min(shft):
          ...
          print("\n As a lot of API calls occur this leads to crashing of the colab notebook and hence no graphs are displayed for the same")
      disp_results(sfd,pa,tpd)
...
**1) Load all file of trades (each simulation = permutation has it’s own file) into colab.**

**2) Split the data for train and test.**

**3) Build a prediction model that based on historical data will decide which model to use per specific hour of the day for the next period of time (week? Day? Month? - your model to decide).**

**4) Create aggregated pn/l  results (with a list of trades of the predicted simulation for the OOS) of the rolling prediction (out of sample) using cross validation.**

**In the process of combining the various stages , the resultant models could be categroised into four possile types they are** 

**1) Trading Analysis based on user input and Super Learner model (without cross validation score)**

**2)Trading Analysis based on automated input and Super Learner model (without cross validation score)**

**3)Trading Analysis based on user input and Super Learner model (with cross validation score)**

**4)Trading Analysis based on automated input and Super Learner model (with cross validation score)**

**Required lIbraries to execute Stage-3 Algorithms directly**
"""
...
"""###**Trading Analysis using Profit/Loss**
...
"""###**Trading Analysis usinwithout using Profit/Loss as inputs**

**Model-1:Trading Analysis based on user input and Super Learner model (without cross validation score)**
"""

def user_preprocess():# processing the inputs for initialsing Stage-1
  df1 = pd.read_csv('/content/GC AT IPS(2)+TM(A) 2016-05_2021 sim131 Trades.txt',delimiter="\t")
  df1['SIM']="sim131"
  df1=df1.dropna(how='any')
  df2 = pd.read_csv('/content/GC AT IPS(2)+TM(A) 2016-05_2021 sim133 Trades.txt',delimiter="\t")
  ...
  print("\n The processed dataframe would be")
  display(df)
  print("\n The different days which the algorithm could process would be ",df['DOW'].unique())
  welcome(df)
...
## User Input algorithm given below
def time_valid(tim): # Function to check validity of time
# inputs: time(str)
# ouptut: boolean value
  try:
...
def welcome(df): #  function to initialize
  print("\n Welcome to Stock Analysis")
  print("\n Kindly enter the details with care below")
  day=input("\n Enter the day of the week")
  f1=['monday','tuesday','wednesday','thursday','friday','sunday','Monday','Tuesday','Wednesday','Thursday','Friday','Sunday','Saturday','saturday']
  if day in f1:
    day=day.capitalize()
    if day =='Sunday' or day == 'sunday':
      tim=input("\n Enter the time  in local time format only  (hrs:min:sec) followed by am/pm after a space")# Enter time in normal method like 01:00:00 am or 02:50:00pm and not in 24 hr clock format
      tmi=tim[0:8]
      if time_valid(tmi):
        tm=hour_format(tim)
        print("\n Conversion of the time in 24 hours format (if in pm) hence the time after conversion would {}".format(tm[0:8]))# converting to 24 hr format for ease of comparision with the primary permutation df
        ...
        if mn>1000:
          print("\n Calculation of the time in Minutes based on the above performed conversion hence the time in minutes would be {}".format(mn))# useful for comparing with the entry time denoted in minutes (converted from 24 hr clock format)
          sim=gather_details(mn,day,df)
          print("\n The best results for the day {} of the week  during time {} of the day is given by {} the permutation (dataset)".format(day,tim,sim))
          bp=get_dataframe(sim)
          bp=bp_process(bp)
           slm_preprocess(bp,df)
        else:
          print("\n Data not valid")
          sys.exit()
      else:
...
def slm_preprocess(bp,df):# processing the inputs for initialsing Stage-2
  df=df.drop(["Date",	"Entry Price",	"Exit Price",	"Trade Quantity",	"Profit/Loss (C)",	"Duration",	"High Price While Open"	,"Low Price While Open",	"Total Efficiency","SIM","Day","Month","year",	"Time(Entry)"	,"DOW"], axis = 1, inplace = True)
  print("\n The unprocessed dataframe would be")
  display(bp)
  bp=bp.set_index('Date')
...
## Super learner Model without cross validation algorithm given below
def CorrectColumnTypes(bp):# Ensure column types are correct
  # Input: dataframe 
  # ouptut: dataframe (with column types changed)
...
def main():# Main Function which calls the user_preprocessing function
  user_preprocess()

main()

"""**Model-2:Trading Analysis based on automated input and Super Learner model (without cross validation score)**"""
...
## Automation algorithm given below
def time_valid(tim): # Function to check validity of time
# inputs: time(str)
# ouptut: boolean value
  try:
...
def welcome(df): #  function to initialize
  ...
  if day in f1:
    ...
    if day =='Sunday' or day == 'sunday':
      ...
      tim=input("\n Enter the time  in the local time format only (hrs:min:sec) followed by a space, followed by am/pm")# Enter time in normal method like 01:00:00 am or 02:50:00pm and not in 24 hr clock format
      tmi=tim[0:8]
      if time_valid(tmi):
        tm=hour_format(tim)
        print("\n Conversion of the time in 24 hours format (if in pm) hence the time after conversion would {}".format(tm[0:8]))# converting to 24 hr format for ease of comparision with the primary permutation df
...
## Super learner Model with cross validation algorithm given below
def CorrectColumnTypes(bp):# Ensure column types are correct
  # Input: dataframe 
  # ouptut: dataframe (with column types changed)
...
"""**Model-4:Trading Analysis based on automated inputs and Super Learner model (with cross validation score)**"""
...
**1) The graphs for each day could be displayed ,but due to the fact that the google colab notebook gets overloaded and hence the notebook eventually crashes therefore i have adjusted the feature to display  maximum of one graph.**

**2) The SuperLearner model was built to house 8 different models (MLP Regression(ANN),Bagging Regression,Extra Tree Regression,Linear Regression,K-Neareast Neighbors Regression,Decision Tree Regression,Random Forest Regression and Ada Boost Regression) but due to time constraints hence three models were selected and are currently housed under the Super Learner model.**

**3) The Above four independent models are a direct result of commbination of each model in stage -1 to a model in stage-2 with appropriate data preprocssing techniques.**

**4) For the algorithms which involve cross validation analysis the number of folds could be adjusted by changing the value of the time folds under each model function.**

**5) Further data (permutation files) could be added into the model and then in such a case the algorithm needs a slight modification of includng the dataframes.**

**6) Furthermore under Stage-2 and Stage-3 Algorithms Keyboard Interruption error is shown as I have forcefully cancelled the exceution of the algorithm after the completion of a single loop Super Learner Model prediction ( since this algorithm runs recursively on an hourly basis).**

**6) Since Sunday and Saturday data are less in the data provided(permutations) hence this algorithm wont function properly for the below inputs alone.**

###**Conclusion:**
**Thus an algorithm which chooses the best permutation out of the given permutations with which a best model  would be selected along with the validity of the model from the list of models based on which OOS(Out Of Samples) Prediction followed by aggregate profit/loss analysis ,percentage of good trading days has succesfully been performed**
"""